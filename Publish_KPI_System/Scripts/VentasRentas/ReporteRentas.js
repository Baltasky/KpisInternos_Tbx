...
var TablaReporteRentas = function () {
    var table;
    var datatable;
    ...
    var exportButtons = () => {
        const documentTitle = 'Reporte Rentas';
        ...
        }).container().appendTo($('#TablaReporteRentas_buttons'));

        const exportButtons = document.querySelectorAll('#TablaReporteRentas_export_menu [Rentas-export]');
        exportButtons.forEach(exportButton => {
            exportButton.addEventListener('click', e => {
                e.preventDefault();

                const exportValue = e.target.getAttribute('Rentas-export');
                var target = document.querySelector('div[id="TablaReporteRentas_buttons"]');
    ...
    return {
        init: function () {
            table = document.querySelector('#TablaReporteRentas');
...
function GetUnidadesOperativas(DivisionesSelected) {
    ...
    $.ajax({
        url: "/VentasRentas/GetUnidadesOperativas",
        type: "post",
        data: {
            TipoPeticion: 1,
...
function GetEquipos(UniOperativaSeleted) {
    ...
    $.ajax({
        url: "/VentasRentas/GetEquiposRentas",
        type: "post",
        data: {
            TipoPeticion: 1,
